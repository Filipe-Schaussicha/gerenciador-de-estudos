...
## Como usar
### Login
Ao entrar no site, você verá uma tela de login com os campos de usuário e senha. Novos usuários e senha podem ser adicionados por meio do endpoint `/criar_user` via POST, que deve ser descomentado do arquivo `login.py` para ser utilizado.
...
### Timer Pomodoro
Para iniciar o timer basta clicar no botão **Start**, para **resetar** o timer clique no botão da à esquerda, já para **pular** para a próxima fase do pomodoro clique no botão à direita. Lembre-se que ao terminar o timer um som de despertador irá tocar para avisa-lo disso.
Para acessar a aba de estatísticas, clique em **Mostrar estatísticas**, ele irá mostras dois gráficos com tempo gasto por dia e em cada disciplina.
...
Esse é um projeto pessoal, entretanto **não há nenhum problema em outras pessoas usarem** ou criarem forks desse projeto. Eu também estou **aberto a contribuições**, não há regras em específico para isso, porém eu recomendo que **tenham bom senso** na suas pull requests e que evite o uso de IA exceto se você souber o que está fazendo.